      ...
      userRepo = `
        <div class="user_repo">
          <div class="repo_with_stars">
            <a href="${repo.html_url}" target="_blank" class="user_repo-link">${
        repo.name
      }</a>
          ...
          ${
            repo.description
              ? `<small class="repo_description">${repo.description}</small>`
              : ""
          }
        </div>
      `;
...
const inputKeyPress = (event) => {
  if (event.key === "Enter" && showUserInputElem.value.trim() !== "")
    onSearchUser();
};
